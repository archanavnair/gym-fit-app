...
import { SCHEMES, WORKOUTS } from "../utils/delauer";
...
function Header(props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex ietm-center gap-2">
    ...
    <SectionWrapper id={'generate'} header={"generate your workout"} title={['It\'s', 'Huge', 'o\'clock']}>
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) => {
          return (
            <button onClick={() => {
              setMuscles([])
              setPoison(type)
            }} className={'bg-slate-950 border border-blue-400 py-3 rounded-lg hover:border-blue-600 duration-200 px-4' +
      ...
      <div className='bg-slate-950 border border-solid border-blue-400 rounded-lg flex flex-col'>
        <button onClick={toggleModal} className="relative p-3 flex items-center justify-center">
          ...
          <i className="fa-solid absolute right-3 top-1/2 -translat-y-1/2 fa-caret-down"></i>
        </button>

        {showModal && (
          <div className="flex flex-col px-3 pb-3">
            {(poison === 'individual' ? WORKOUTS[poison] :
              Object.keys(WORKOUTS[poison])).map((muscleGroup, muscleGoupIndex) => {
                return (
                  <button onClick={() => {
                    updateMuscles(muscleGroup)
                  }} className={'hover:text-blue-400 duration-200' + (muscles.includes(muscleGroup) ?
                    ' text-blue-400' : ' ')} key={muscleGoupIndex}>
